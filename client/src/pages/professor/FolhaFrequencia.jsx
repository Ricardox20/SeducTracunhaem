...
import { MOCK_DATA } from '../../mocks/data'; // Importando para ler as disciplinas
import { 
...
const FolhaFrequencia = () => {
  ...
  // --- LÓGICA DE DISCIPLINAS (Anos Finais) ---
  const disciplinasDisponiveis = useMemo(() => {
    // Filtra as disciplinas do mock que batem com o nível "Finais"
    if (user?.nivel_ensino === 'Finais') {
      return MOCK_DATA.disciplinasPadrao.filter(d => d.nivel === 'Finais');
    }
    return [];
  }, [user]);
  ...
  const handleSalvar = () => {
    if (user?.nivel_ensino === 'Finais' && !disciplinaSelecionada) {
        alert("Por favor, selecione a disciplina antes de salvar a frequência.");
        return;
    }
    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="p-3 bg-white rounded-2xl border border-slate-100 text-slate-400 shadow-sm hover:text-primary transition-all">
            <ArrowLeft size={20} />
          </button>
          <div>
            ...
            <p className="text-[10px] font-black text-primary uppercase mt-1">
              {modoVisao === 'diario' ? 'Chamada Diária' : 'Lançamento Mensal'}
            </p>
          </div>
        </div>
      ...
      {/* SELETOR DE DISCIPLINA (Apenas Anos Finais) */}
      {user?.nivel_ensino === 'Finais' && (
        <div className="bg-amber-50 border border-amber-100 p-4 rounded-3xl flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-[9px] font-black text-amber-600 uppercase mb-1">Disciplina da Aula</p>
            <select 
              value={disciplinaSelecionada} 
              onChange={(e) => setDisciplinaSelecionada(e.target.value)}
              className="bg-transparent border-none text-xs font-black uppercase focus:ring-0 w-full p-0"
            >
      ...
      {modoVisao === 'diario' ? (
        <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-6 bg-slate-50/50 flex items-center justify-between border-b">
            ...
            <div className="flex items-center gap-2 text-[9px] font-black text-slate-400 uppercase">
              <CheckCircle size={14} className="text-emerald-500"/> Padrão: Todos Presentes
            </div>
          </div>
          ...
          {!isFinalDeSemana(dataSelecionada) ? (
              ...
              {alunosDaTurma.map((aluno) => (
                <div key={aluno.id} className="p-4 px-8 flex items-center justify-between hover:bg-slate-50/30">
                  ...
                  <button 
                    ...
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-white transition-all shadow-md ${
                      chamadaDia[aluno.id]?.status === 'P' ? 'bg-emerald-500 shadow-emerald-100' : 
                      chamadaDia[aluno.id]?.status === 'F' ? 'bg-red-500 shadow-red-100' : 'bg-amber-500 shadow-amber-100'
                    }`}
                  >
            ...
            <div className="p-20 text-center"><Coffee size={40} className="mx-auto text-slate-200 mb-2"/><p className="text-xs font-black text-slate-300 uppercase">Fim de semana</p></div>
          )}
        </div>
      ) : (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-6 bg-slate-50/50 border-b flex justify-between items-center">
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Preenchimento Manual por dia</p>
          </div>
