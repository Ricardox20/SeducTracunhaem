...
export default function DiarioClasse() {
    ...
    if (loading) return <div className="p-8 text-center text-seduc-primary">Carregando diário...</div>;
    ...
    return (
        <div className="animate-fadeIn pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <button onClick={() => navigate('/portal-professor')} className="flex items-center text-gray-500 hover:text-seduc-primary mb-2 text-sm transition">
                        <ArrowLeft size={16} className="mr-1" /> Voltar para Turmas
                    </button>
                    <h1 className="text-2xl font-bold text-seduc-primary">{contexto?.disciplina}</h1>
            ...
            <div className="flex gap-4 border-b border-gray-200 mb-6">
                <button
                    ...
                    className={`pb-3 px-2 flex items-center gap-2 font-medium transition border-b-2 ${activeTab === 'diario' ? 'border-seduc-primary text-seduc-primary' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
                >
                    <IconList size={18} /> Diário de Classe
                </button>
                <button
                    onClick={() => setActiveTab('estatisticas')}
                    className={`pb-3 px-2 flex items-center gap-2 font-medium transition border-b-2 ${activeTab === 'estatisticas' ? 'border-seduc-primary text-seduc-primary' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
                >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fadeIn">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="date" {...register('data_aula')} className="w-full p-2 border border-gray-200 rounded outline-none focus:border-seduc-primary" required />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Conteúdo Ministrado</label>
                                    <input type="text" {...register('conteudo')} placeholder="Ex: Introdução à Anatomia Humana..." className="w-full p-2 border border-gray-200 rounded outline-none focus:border-seduc-primary" />
                                </div>
                            </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button type="submit" className={`flex-1 ${aulaEditandoId ? 'bg-orange-600 hover:bg-orange-700 text-white' : 'bg-seduc-primary text-white hover:bg-white hover:text-seduc-primary border border-transparent hover:border-seduc-primary'} font-bold py-3 rounded-lg transition flex items-center justify-center gap-2`}>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 h-fit">
                        <h3 className="font-bold text-gray-700 mb-4 flex items-center gap-2">
                            <Calendar size={18} className="text-seduc-primary" /> Histórico de Aulas
                        </h3>
                        {historico.length === 0 ? (
                            ...
                            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                                {historico.map(aula => (
                                    <div key={aula.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100 hover:border-seduc-primary transition group relative">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className="text-xs font-bold text-seduc-primary uppercase">
                                            ...
                                            <div className="flex gap-1 ml-2">
                                                ...
                                                <button
                                                    ...
                                                    className="text-gray-300 hover:text-seduc-primary p-1 transition"
            ...
            {aulaVisualizando && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-scaleIn">
                        ...
                        {/* Cabeçalho do Modal */}
                        <div className="bg-seduc-primary p-4 flex justify-between items-center text-white">
                            <h3 className="font-bold text-lg flex items-center gap-2">
                                <Calendar size={20} className="text-white" /> Detalhes da Aula Registrada
                            </h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {aulaVisualizando.frequenciasDetalhadas.map(f => (
                                    <div key={f.id} className="flex items-center justify-between p-2 border border-gray-100 rounded bg-gray-50">
                                        ...
                                        <span className={`text-xs font-bold px-2 py-1 rounded border ${f.status === 'Presente' ? 'bg-green-100 text-green-700 border-green-200' :
